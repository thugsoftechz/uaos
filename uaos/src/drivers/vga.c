...
void vga_set_pixel_char(int x, int y, char c, uint8_t color) {
    if (x < 0 || x >= VGA_WIDTH || y < 0 || y >= VGA_HEIGHT) return;

    // Check if we want to preserve background?
    // Usually standard VGA behavior is Foreground | Background
    // If color is just 0-15, it implies foreground with current background?
    // But for this simplified OS, let's assume 'color' is the full attribute if it's > 15
    // OR we can pass fg and bg separately.
    // Given the previous usage in gui.c, I passed 'color' which is usually an enum vga_color (0-15).
    // So I should assume Black background? Or White?
    // In gui_draw_icon I passed vga_color.

    // Let's implement smart logic:
    // If color is small (0-15), assume it is foreground on Transparent (preserve BG).
    // But reading BG is slow? No, it's memory mapped.

    uint16_t current = vga_buffer[y * VGA_WIDTH + x];
    uint8_t bg = (current >> 12) & 0x0F;
    uint8_t attribute = (bg << 4) | (color & 0x0F);

    vga_buffer[y * VGA_WIDTH + x] = (uint16_t)((uint8_t)c) | ((uint16_t)attribute << 8);
}
