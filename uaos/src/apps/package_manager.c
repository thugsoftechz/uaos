...
int package_manager_main(void) {
    if (current_language == LANG_HINGLISH)
        ...
        gui_create_window("UPack - Software Center", 45, 2, 32, 15);
...
void package_manager_load_package_list(void) {
    ...
    if (current_language == LANG_HINGLISH) {
        ...
        strcpy(packages[1].name, "Free Recharge");
        packages[1].installed = true;
        package_count++;

        strcpy(packages[2].name, "Download RAM");
        packages[2].installed = false;
        package_count++;
    } else {
        strcpy(packages[0].name, "Chrome (Ram Eater)");
        packages[0].installed = false;
        package_count++;

        strcpy(packages[1].name, "System Cleaner");
        packages[1].installed = true;
        package_count++;

        strcpy(packages[2].name, "More RAM");
        packages[2].installed = false;
        package_count++;
    }
}
...
void package_manager_draw_interface(void) {
    ...
    if (current_language == LANG_HINGLISH)
        gui_draw_text(win->x + 2, win->y + 10, "[Ent] Lelo/Hatao", VGA_COLOR_CYAN);
    else
...
void package_manager_toggle_package(void) {
    ...
    if (packages[selected_package].installed) {
        if (current_language == LANG_HINGLISH)
            gui_show_message("UPack", "Installing... Arre storage full hai!");
        else
            gui_show_message("UPack", "Installing... Error: dependency missing.");
    } else {
        if (current_language == LANG_HINGLISH)
            gui_show_message("UPack", "Removing... Virus nahi jayega.");
        else
            gui_show_message("UPack", "Removing... Core components damaged.");
    }
    package_manager_draw_interface();
}
