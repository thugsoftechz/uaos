...
int media_player_main(void) {
    if (current_language == LANG_HINGLISH)
        ...
        gui_create_window("UMedia - Audio Player", 10, 8, 50, 12);
...
void media_player_load_playlist(void) {
    if (current_language == LANG_HINGLISH) {
        ...
        strcpy(playlist[2].artist, "Bollywood");
    } else {
        strcpy(playlist[0].title, "Sad Song.mp3");
        strcpy(playlist[0].artist, "Unknown Artist");

        strcpy(playlist[1].title, "RickRoll.wav");
        strcpy(playlist[1].artist, "Rick Astley");

        strcpy(playlist[2].title, "Elevator Music.ogg");
        strcpy(playlist[2].artist, "Stock Sound");
    }
}
...
void media_player_draw_interface(void) {
    ...
    char status[64];
    if (current_language == LANG_HINGLISH)
        sprintf(status, "%s", is_playing ? "BAJ RAHA HAI (Full Volume)" : "RUK GAYA (Shanti)");
    else
        sprintf(status, "%s", is_playing ? "PLAYING" : "PAUSED");
    ...
    if (current_language == LANG_HINGLISH)
        ...
        gui_draw_text(win->x + 2, win->y + 8, "[SPC] Play/Pause [N] Next [S] Stop", VGA_COLOR_CYAN);
}
...
void media_player_toggle_play(void) {
    ...
    if (is_playing) {
        if (current_language == LANG_HINGLISH)
            ...
            gui_show_message("UMedia", "Audio output started.");
    }
}
